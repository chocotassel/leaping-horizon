...
> **最低兼容基线：Android 8 WebView。** 不使用该环境不支持的 CSS / JavaScript / Web API；例如禁止 CSS `inset: ...` 简写属性，改写为 `top` / `right` / `bottom` / `left`。使用较新能力时必须提供可用的回退、polyfill 或编译降级，且不能把 PC 模拟器可运行视为真机兼容。
...
4. **跨端适配** — 先读 [cross-platform-h5.md](references/cross-platform-h5.md)：Android 8 WebView 兼容基线、触摸、滚动、安全区、PC vs 真机差异
...
> **产出前提**：交付的 zip 必须最低兼容 Android 8 WebView，并同时满足 `zip-artifact-spec.md`、`device-capabilities.md`、`cross-platform-h5.md` 与（若使用 JSBridge）`jsbridge-api.md` 的全部约束。任何约束以 reference 为准。
...
| [cross-platform-h5.md](references/cross-platform-h5.md) | 适配多端时：Android 8 WebView 兼容、触摸、滚动、安全区、PC 模拟器与真机差异 |
