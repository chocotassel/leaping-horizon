...
test('plans an alternating edge drum run from real timestamps and lane capacities', () => {
  const result = planFullWidthSweeps({
    slots: makeSlots(18),
    mobility: [0, ...Array.from({ length: 17 }, () => 1)],
    laneCount: 5,
    secondsPerBeat: 0.4,
    orientationSeed: 0.2,
  });
  ...
  assert.deepEqual(gesture.anchorLanes, [0, 4, 0]);
  ...
  assert.equal(gesture.anchorSlots.every((slot) => slotsAreDownbeats(slot)), true);
  assert.equal(gesture.edgeToEdgeTransitionCount, 2);
...
test('keeps blocked gesture slots out of a sweep window', () => {
  ...
  const result = planFullWidthSweeps({
    slots,
    mobility: [0, ...Array.from({ length: 33 }, () => 1)],
    laneCount: 5,
    secondsPerBeat: 0.4,
  });
...
function slotsAreDownbeats(slotIndex) {
  return slotIndex % 4 === 0;
}
